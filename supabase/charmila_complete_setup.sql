-- ============================================================
-- CHARMILA COMPUTERS — COMPLETE SUPABASE END-TO-END SETUP SQL
-- Run this query once in Supabase SQL Editor:
-- https://supabase.com/dashboard/project/genhheydpoywoqhdavmy/sql/new
-- ============================================================
...
create policy "profiles: admins read all" on public.profiles for select using (
...
create policy "products: admins write" on public.products for all using (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.is_admin)
);


-- 3B. STORE SETTINGS TABLE (active_theme)
...
create policy "store_settings: admins write" on public.store_settings for all using (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.is_admin)
);
...
create policy "banners: admins write" on public.banners for all using (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.is_admin)
);
...
create policy "orders: admins update" on public.orders for update using (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.is_admin)
);
...
drop policy if exists "order_items: read own order or admin" on public.order_items;
create policy "order_items: read own order or admin" on public.order_items for select using (
  exists (
    select 1 from public.orders o
    where o.id = order_items.order_id
      and ((o.user_id is not null and auth.uid() = o.user_id) or
           exists (select 1 from public.profiles p where p.id = auth.uid() and p.is_admin))
  )
);
...
-- 6. PROMOTE ADMIN USER
insert into public.profiles (id, full_name, is_admin)
select id, 'Charmila Admin', true
from auth.users where email = 'admin@charmilacomputers.in'
