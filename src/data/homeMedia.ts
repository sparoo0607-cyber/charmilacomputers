 ...
 */
export function getThemeMedia(themeId: "festive" | "standard"): HomePageMediaState {
  return themeId === "standard" ? STANDARD_THEME_MEDIA : VINAYAKA_THEME_MEDIA;
}

/**
 * Applies a theme across all homepage images, banners, and categories, saving to Server API, Supabase & localStorage
 */
export async function applyThemeMedia(themeId: "festive" | "standard"): Promise<HomePageMediaState> {
 ...
 */
export async function loadHomeMedia(themeOverride?: "festive" | "standard"): Promise<HomePageMediaState> {
  let resolvedTheme: "festive" | "standard" = themeOverride || "standard";
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (savedSettings) {
          ...
          if (parsed.activeTheme) resolvedTheme = parsed.activeTheme;
        }
      } catch {}
    }
    try {
      ...
      if (res.ok) {
        ...
        if (json.activeTheme) {
          resolvedTheme = json.activeTheme;
        }
      }
    } catch {}
  }
      ...
      if (saved) {
        ...
        const isClean = (url?: string) => {
          ...
          if (resolvedTheme === "standard") {
            return !url.includes("/themes/vinayaka/") && !url.includes("/images/festive/");
          }
          return !url.includes("/themes/standard/");
        };
 ...
 */
export async function saveHomeMedia(media: HomePageMediaState, themeId?: "festive" | "standard"): Promise<void> {
  let currentTheme: "festive" | "standard" = themeId || "standard";
  ...
  if (!themeId && typeof window !== "undefined") {
    try {
      ...
      if (savedSettings) {
        const parsed = JSON.parse(savedSettings);
        if (parsed.activeTheme) currentTheme = parsed.activeTheme;
      }
    } catch {}
  }
